...
void main_algorithm(SND_data * robot)
{
	try
	{
		...
		double fMaxRange = robot->GetMaxRange();
		double fScanRes = robot->GetScanRes();
		int iNumLPs = robot->GetCount();
...
*/            
            double goalX = robot->getGoalX();
            double goalY = robot->getGoalY();
            double goalA = robot->getGoalA();
...
*/
			...
			distToGoal = std::max(0.01,sqrt( pow(goalX - robot->GetXPos(),2) + pow(goalY-robot->GetYPos(),2)));
			radToGoal = normalize(atan2( (goalY-robot->GetYPos())/distToGoal, (goalX-robot->GetXPos())/distToGoal ) - robot->GetYaw());
...
//					pp.SetSpeed(0.0, 0.0);
				...
				{
					newTurnRate = limit((goalA - robot->GetYaw())/3, -maxTurnRate, maxTurnRate);
					if( gDebug > 4 ) 
					{
						std::cout<< "Spinning to goal angle " << goalA << " from " << robot->GetYaw(); 
...
//			pthread_mutex_lock(&(pp.goal_mutex));
			distToGoal = sqrt( pow(robot->getGoalX() - robot->GetXPos(),2) + pow(robot->getGoalY()-robot->GetYPos(),2));
//			pthread_mutex_unlock(&(pp.goal_mutex));
...
*/
			robot->publishSpeed( newSpeed, newTurnRate );
